...
export function DeckCard({ deck }: DeckCardProps) {
  const color = "from-red-500 to-red-700"
  return (
    <Link to={`/deck/${deck.id}`}>
      <div className="group relative overflow-hidden rounded-2xl bg-white shadow-md active:shadow-lg transition-all duration-200 active:scale-[0.98] border border-gray-100">
        ...
        <div className="relative p-5">
          ...
          <h3 className="text-xl mb-1.5">{deck.nome}</h3>
          <p className="text-sm text-gray-600">Deck de estudos</p>
          
          <div className="mt-3">
            <div className="bg-gray-200 rounded-full h-1.5 overflow-hidden">
              <div className={`h-full bg-gradient-to-r ${color} w-1/3`} />
            </div>
          </div>
        </div>
      </div>
